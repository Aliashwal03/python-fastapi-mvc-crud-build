...
@router.get('/comments', response_model=CommentSchema)
def get_all_comments(comment_id: int, db: Session = Depends(get_db)):
...
def delete_tea(comment_id: int, db: Session = Depends(get_db)):
    db_comment = db.query(CommentModel).filter(CommentModel.id == comment_id).first()
    if not db_comment:
